...
module.exports.addMusic = (win) => {
    ...
    result.then((e) => {
        ...
        fs.readFile('app\\shared\\music.json', 'utf8', (err, data) => {
            ...
            console.log('Contenuto del file JSON:', musicList);

            // Aggiungi il nuovo filePath alla lista
            musicList.push(filePath);

            // Scrivi di nuovo il file JSON aggiornato
            fs.writeFile('app\\shared\\music.json', JSON.stringify(musicList, null, 2), 'utf8', (writeErr) => {
            if (writeErr) {
                console.error('Errore nella scrittura del file JSON:', writeErr);
                return;
            }
